use crate::{
    ClipboardAction, ClipboardEvent, EventMetadata, HotkeyEvent, KeyboardEvent, MouseButton, MouseEvent, MouseEventType, Position, Result, UiFocusChangedEvent, UiPropertyChangedEvent, WorkflowEvent, WorkflowRecorderConfig
};
use std::{
    sync::{Arc, Mutex},
...
use windows::Win32::UI::WindowsAndMessaging::{
    GetMessageW, TranslateMessage, DispatchMessageW, MSG, PostThreadMessageW, WM_QUIT
};
...
impl WindowsRecorder {
                ...
                match event.event_type {
                    EventType::KeyPress(key) => {
                        ...
                        if capture_ui_elements {
                            ...
                            ui_element = Self::get_focused_ui_element(&automation);
                        }
    ...
    fn setup_ui_automation_events(&self) -> Result<()> {
        ...
        thread::spawn(move || {
            ...
            let com_initialized = unsafe {
                ...
                if hr.is_ok() {
                    info!("✅ Successfully initialized COM apartment as STA for UI Automation events");
                    true
                    ...
                    error!("❌ Failed to initialize COM apartment for UI Automation: {:?}", hr);
                    return;
                }
            };
            ...
            *ui_automation_thread_id.lock().unwrap() = Some(thread_id);
            ...
            let automation = match uiautomation::UIAutomation::new_direct() {
                ...
                Err(e) => {
                    ...
                    warn!("UI Automation events will be disabled, but other recording will continue");
            ...
            if record_property_changes {
                ...
                let property_handler: Box<uiautomation::events::CustomPropertyChangedEventHandlerFn> = Box::new(move |sender, property, value| {
                    ...
                    match property {
                        ...
                        uiautomation::types::UIProperty::HasKeyboardFocus => {
                            debug!("Property change event received for {:?} on thread {}", property, current_thread);
                        }
                        _ => {
                ...
                match automation.get_root_element() {
                    ...
                    Err(e) => error!("❌ Failed to get root element for property change events: {}", e),
                }
    ...
    pub fn stop(&self) -> Result<()> {
        ...
        if let Some(thread_id) = *self.ui_automation_thread_id.lock().unwrap() {
            unsafe {
                let result = PostThreadMessageW(thread_id, WM_QUIT, 
                    WPARAM(0), 
                    LPARAM(0));
                if result.is_ok() {
                    debug!("Posted WM_QUIT message to UI Automation thread {}", thread_id);
                } else {
                    warn!("Failed to post WM_QUIT message to UI Automation thread {}", thread_id);
                }
            }
        }
