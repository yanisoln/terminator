...
use terminator::{AutomationError, Selector}; // Import Element
...
fn main() -> Result<()> {
    ...
    let pharma_app = engine
        .get_application_by_name("pharma.com - v0 App")
        .unwrap();
    pharma_app.focus()?;
    ...
    let radio_click_fields = vec!["insuranceYes", "medicareNo", "medicaidNo"];
    ...
    for input_field in input_fields {
        ...
        let automation_id =
            if automation_id_raw.starts_with("STRING(") && automation_id_raw.ends_with(')') {
        ...
        println!(
            "input field raw: {:?}, parsed: {:?}",
            automation_id_raw, automation_id
        ); // Updated log

        let text_to_type = text_fields
            .iter()
            .find(|(id, _)| {
        ...
        if let Err(e) = input_field.type_text(text_to_type) {
            println!(
                "error typing text ('{}') into input field ({}): {:?}",
                text_to_type, automation_id, e
            );
        }
        println!(
            "typed '{}' into input field id: {}",
            text_to_type, automation_id
        ); // Updated log
        thread::sleep(Duration::from_millis(10));
    }

    // get all radio buttons and click the specified ones
    let radio_buttons = pharma_app
        .locator(Selector::Role {
            role: "radiobutton".to_string(),
            name: None,
        })
        .unwrap()
        .all()
    ...
    for radio_button in radio_buttons {
        ...
        if radio_click_fields.contains(&automation_id) {
            // Use the new radio_click_fields vector
            println!("attempting to click radio button: {}", automation_id);
            if let Err(e) = radio_button.focus() {
                println!(
                    "error focusing on radio button ({}): {:?}",
                    automation_id, e
                );
            }
            if let Err(e) = radio_button.hover() {
                println!(
                    "error hovering on radio button ({}): {:?}",
                    automation_id, e
                );
            }
    ...
    for checkbox in checkboxes {
        ...
        if checkbox_click_fields.contains(&automation_id) {
            // Use the new checkbox_click_fields vector
